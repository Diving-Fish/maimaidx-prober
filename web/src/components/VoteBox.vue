<template>
  <v-dialog width="500px" :fullscreen="$vuetify.breakpoint.mobile" v-if="visible" v-model="open">
    ...
    <v-card :loading="loading">
      ...
      <div v-if="remain <= 0" style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100px;">
        <span class="mb-4">投了五组啦，休息一下明天再来吧！</span>
        <v-btn @click="getNewVote(true)">我偏要继续</v-btn>
      </div>
    </v-card>
  </v-dialog>
</template>
