...
> **特殊说明：同名歌曲 "Link"**
>
> maimai DX 中存在两首同名歌曲 "Link"（id 131 与 id 383）。在数据库中，id 131 的歌曲标题为 `Link`，而 id 383 的歌曲标题为 `Link(CoF)`。因此，上传 id 383（*Link* by 立秋 feat.ちょこ）的成绩时，`title` 字段应填写 `Link(CoF)` 而非 `Link`，否则该上传会被识别为 id 131 的成绩或被跳过。
